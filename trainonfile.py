...
chain.train_on_file(filename='training_txt/codinginterview.txt', verbose=True)

print(f'len(chain.tree): {len(chain.tree)}\n')

print('Adjusting weights. This may take a while.\n_\n')
chain.bulk_adjust_weights(fitness_functions=[aw_mult(aw_favor_complexity, .001), aw_mult(aw_favor_punctuation, .00015), aw_mult(aw_favor_alternating_complexity, .1)], iterations=len(chain.tree)//4)
